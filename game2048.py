...
class Game2048(base.Environment):
  ...
  def _StackRow(row):
    ...
    return np.array(result + [0] * (4 - len(result)))
  ...
  def ProcessAction(self, action):
      ...
      if action == 0:  # up
        for i in range(4):
          self._state[:, i] = Game2048._StackRow(self._state[:, i])
      elif action == 1:  # down
        for i in range(4):
          self._state[::-1, i] = Game2048._StackRow(self._state[::-1, i])
      elif action == 2:  # left
        for i in range(4):
          self._state[i, :] = Game2048._StackRow(self._state[i, :])
      elif action == 3:  # right
        for i in range(4):
          self._state[i, ::-1] = Game2048._StackRow(self._state[i, ::-1])
